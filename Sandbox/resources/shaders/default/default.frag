...
void main() {
	...
	vec3 scatteredLight = ambient + lightColor * diff;
	vec3 reflectedLight = lightColor * spec * strength;

	vec3 result = min(color.rgb * scatteredLight + reflectedLight, vec3(1.0));
