...
namespace raytracy {
	...
	void Raytracer::Submit(const HitableCollection& objects, const Camera& camera,
	...
	Color3 Raytracer::ComputeRayColor(const Ray& ray,
		const HitableCollection& objects,
		uint32_t depth) {
		Hit hit;
		if (depth <= 0) {
			return Color3(0.0f);
